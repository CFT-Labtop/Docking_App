...
class FlutterRouter extends BaseRouter {
  ...
  void configureRoutes() {
    ...
    this.fluroRouter.define("/" + Pages("NewBookingPage").getName() + "/:warehouse/:warehouseName",
        handler: Handler(handlerFunc: (context, params){
          final shipmentList = context.settings.arguments as List<String>;
          return NewBookingPage(warehouseID: int.parse(params["warehouse"][0]), shipmentList:shipmentList, warehouseName: params["warehouseName"][0],);
        }));
    this.fluroRouter.define("/" + Pages("ScanQRCodePage").getName(),handler: Handler(handlerFunc: (context, params) => ScanQRCodePage()));
    this.fluroRouter.define("/" + Pages("ConfirmBookingPage" + "/:truckTypeName/:clientTypeName/:truckCompanyName").getName(),handler: Handler(handlerFunc: (context, params) {
      // final Booking booking = context.settings.arguments as Booking;
      final Map<String, dynamic> map = context.settings.arguments;
      return ConfirmBookingPage(booking: map["booking"], truckTypeName: params["truckTypeName"][0],timeSlot: map["timeSlot"], clientTypeName: params["clientTypeName"][0], truckCompanyName: params["truckCompanyName"][0],);
    }));
  }
}
